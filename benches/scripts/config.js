...
export const frameworks = [
	{
		label: "signals-master",
		dependencies: {
			preact: "file:" + repoRoot("node_modules/preact"),
			framework:
				"file:" + repoRoot("benches/proxy-packages/signals-master-proxy"),
		},
		async isValid() {
			try {
				await stat(repoRoot("preact-signals.tgz"));
	...
	},
	{
		label: "signals-local",
		dependencies: {
			// preact: "npm:preact@latest",
			preact: "file:" + repoRoot("node_modules/preact"),
			// preact: "file:" + repoRoot("benches/proxy-packages/preact-proxy"),
			// framework: "file:" + repoRoot("benches/proxy-packages/signals-local-proxy"),
			framework: "file:" + repoRoot("packages/preact"),
		},
		isValid() {
			return validateFileDep(this.dependencies.framework);
		},
...
function getBaseBenchmarkConfig(benchPath, options) {
	...
	let url = path.posix.relative(toUrl(benchesRoot("dist")), toUrl(benchPath));
