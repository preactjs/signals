...
if (!window.__PREACT_SIGNALS_CONTENT_SCRIPT_INJECTED__) {
	window.__PREACT_SIGNALS_CONTENT_SCRIPT_INJECTED__ = true;
	...
	(function () {
		...
		function connectToBackground() {
			...
			try {
				connectionAttempts++;
				connectionPort = chrome.runtime.connect({
		...
		function handleMessageFromDevTools(message) {
			...
			switch (type) {
				...
				case "PING":
					// Respond to ping to indicate content script is alive
					if (connectionPort) {
						try {
				...
				case "HIGHLIGHT_SIGNALS_PANEL":
					// This is a UI hint message, can be ignored in content script
					break;

				default:
					// Only log truly unhandled messages in development
					if (
						typeof message.type === "string" &&
						!message.type.startsWith("_")
					) {
